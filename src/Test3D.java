...
public class Test3D {
	...
	public void run() {
		...
		// glEnable bringt, dass eine bestimmte Sache aktiv wird. In diesem Fall
		// der Depth Test.
		// Der Depth Test bringt, mach das die Depth aktiv wird.
		// Den MUSS man aktivieren wenn man das in 3D machen will. Mehr dazu
		// unten im Code.
		...
		// FOV sollte glaube ich jeder kennen!
		// (http://de.wikipedia.org/wiki/FOV)
		...
		while (!Display.isCloseRequested()) {
			...
			// Was timeSinceLastFrame (tslf) ist hat Brotcrunsher schon
			// erklärt.
			float tslf = ((float) now - last) / 1000f;
			// Hier müssen wir ein | machen (Strg+Alt+>) und dann noch die
			// Pixel die in der Depth (Tiefe) sind zu löschen.
			// Ups.. mir fällt gerade auf das ich diesen Befehl noch nicht
			// erklärt habe. Das hole ich mal nach.
			// glClear mach das die Pixel gelöscht werden also auf Schwarz
			// gesetzt werden.
			...
			// Das zweite in welcher Richtung y rotiert werden soll (0 = gar
			// nicht, 1 = im Uhrzeigersinn, -1 = gegen den Uhrzeigersinn)
